...
describe('便利贴与窗台期盼的分寸措辞', () => {
    it('便利贴摆出来的同时说清「记着不等于要一直说」', async () => {
        ...
        expect(out).toContain('Pinned Notes (important near-term matters)');
        expect(out).toContain('小明后天要考试');
        // 对症的三件事：别每轮都说、别追问进展、别替对方安排时间
        expect(out).toContain('Keeping something in mind does not mean saying it constantly');
        expect(out).toContain("Don't chase progress on the same matter every chat");
        expect(out).toContain("don't schedule when they should get around to it");
    });
    ...
    it('窗台期盼同样带分寸，别被当成待办清单', async () => {
        ...
        expect(out).toContain('Windowsill Hopes');
        expect(out).toContain('想一起去看海');
        expect(out).toContain('not a to-do list');
        expect(out).toContain('no need to bring them up every time');
    });
