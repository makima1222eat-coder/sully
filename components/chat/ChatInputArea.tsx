...
import { ShareNetwork, Trash, Plus, Smiley, PaperPlaneTilt, Money, BookOpenText, GearSix, Image, Lock, ArrowsClockwise, ChatCircleDots, CalendarBlank, ForkKnife, Coffee, Code, Brain, PencilSimple, BellSimpleRinging, Alarm, Sparkle, FadersHorizontal, LinkSimple, Heart, Star } from '@phosphor-icons/react';
                    ...
                    {showPanel === 'actions' && !actionsContent && (
                        ...
                        >
                          ...
                          <div className={`p-6 grid grid-cols-4 gap-8 ${actionsPage === 1 ? '' : 'hidden'}`}>
                            ...
                            {/* 「心声」按钮：tap → 直接开关（无设置弹窗）。开启后角色每轮回复前多一行可折叠的内心独白 */}
                            <button
                              onClick={() => onPanelAction('inner-voice-toggle')}
                              className={`flex flex-col items-center gap-2 active:scale-95 transition-transform ${acnh ? 'text-[#725d42]' : isDiscordStyle ? 'text-slate-200' : 'text-slate-600'}`}
                            >
                              <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-sm border relative ${
                                  innerVoiceEnabled
                                  ...
                                  {innerVoiceEnabled && <span className={`absolute -top-1 -right-1 w-3 h-3 rounded-full border-2 ${isDiscordStyle ? 'bg-rose-400 border-slate-900' : 'bg-rose-500 border-white'}`} />}
                              </div>
                              <span className="text-xs font-bold">{innerVoiceEnabled ? '心声已开' : '心声'}</span>
                            </button>
