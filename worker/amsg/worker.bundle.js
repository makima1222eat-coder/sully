...
// utils/memoryPalace/userPronoun.ts
function buildUserPronounRule(userLabel) {
  return `**Pronouns for ${userLabel}**: Refer to ${userLabel} with she / her / hers / herself only. Never use they / them / their / themself for ${userLabel}, and never use it as a gender-neutral fallback when the source material does not state ${userLabel}'s gender. Pronouns for everyone else are unaffected: keep third parties as the source describes them.`;
}
...
var ROOM_RULES = {
  user_room: `Imagine writing a **character card** for the other person. Only information that must appear on that card belongs here: basic identity, broad profession, residence, family structure, significant people (for example, "Her friend Mei: a close university roommate"), core facts established through long-term interaction, and life events major enough to shape the person, such as bereavement or emigrating. The threshold is extremely high; prefer omission over clutter. Exclude temporary states, emotional analysis, personality profiling, and ongoing events without conclusions.`,
  self_room: `Stable knowledge about **myself**: who I am, my underlying disposition, important changes, and insights I have internalized. Exclude opinions about other people.`,
  bedroom: `The **texture** of our relationship: shared habits and rituals, private jokes, unspoken understanding, and real but uncertain feelings. Hard rule: never name or classify the relationship. Do not write definitions such as "we are lovers, partners, friends, or family." Describe only observable patterns and feelings; uncertainty itself may be retained, for example, "I cannot define us, but I am the first person she seeks when she is hurting."`,
  study: `My domains: what I can do, what I am learning, and what we study together. Retain accumulated knowledge, not one-off topics.`
};
...
var PLATE_USER_TURN = "Begin organizing the plates.";
...
var normalizeNewsRegion = (region) => region === "GB" || region === "ALL" ? region : "CN";
var getNewsScopePlatforms = (region, platforms) => region === "CN" ? resolveHotNewsPlatforms(platforms) : [`brave:${region}`];
...
var fetchBraveRegionalNews = async (opts) => {
  ...
  try {
    ...
    return results.filter((item) => item && item.title).slice(0, count).map((item) => {
      const rawDesc = typeof item.description === "string" ? item.description : typeof item.snippet === "string" ? item.snippet : "";
      const desc = rawDesc.replace(/\s+/g, " ").trim();
      ...
      return {
        ...
        url: item.url,
        desc: desc && desc !== title ? desc : void 0
      };
    });
  } catch (e) {
...
var loadHotNews = async (cfg, nowMs, globalRows, pendingWrites) => {
  ...
  const fresh = region === "CN" ? await fetchHotNews(cfg.newsPlatforms, 12, HOTNEWS_KEEP) : await fetchBraveRegionalNews({
  ...
  if (snap && snap.items.length > 0 && sameHotNewsPlatforms(snap.platforms, platforms) && nowMs - snap.fetchedAt <= HOTNEWS_FALLBACK_MAX_AGE_MS) {
...
var TIME_FRAMING_CONVERSATIONAL = "Time is the backdrop you are in right now: it seeps into your tone, your state, the things you mention in passing. As for where this conversation goes and whether it continues, follow whatever you two are actually talking about \u2014 a topic finds its own natural end. That they are still talking with you at this hour is itself their choice.";
...
var stripInnerVoiceLines = (t) => t.replace(/^[ \t]*[\[【]\s*心声\s*[\]】][:：]?[ \t]*.*$/gm, "").replace(/\n{3,}/g, "\n\n");
var stripSourceTags = (t) => t.replace(/\s*\[(?:聊天|通话|约会|Chat|Call|Date)\]\s*/gi, "\n").replace(/\s*\[(?:Story|剧情)[:：][^\]\n]{0,80}\]\s*/gi, "\n");
